...
[v-stock Guided Tour][v-stock-yt]

[v-stock-yt]: https://www.youtube.com/playlist?list=PLx0iOsdUOUmn7D5XL4mRUftn8hvAJGs8H
...
```
git remote add v-stock https://github.com/FullstackAcademy/v-stock.git
git fetch v-stock
git merge v-stock/master
```

Why did we do that? Because every once in a while, `v-stock` may
...
```
export MY_APP_NAME=v-stock
createdb $MY_APP_NAME
createdb $MY_APP_NAME-test
```

* By default, running `npm test` will use `v-stock-test`, while
  regular development uses `v-stock`
...
v-stock comes with a working linter (ESLint, with
...
supported ways to deploy in v-stock:
...
v-stock comes with a `.travis.yml` configuration almost ready for
...
using v-stock's handy deployment script:
